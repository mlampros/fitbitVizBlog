...
CRS_value = as.integer({{CRS_value}})
asc_desc_linestring = as.character("{{asc_desc_linestring}}")
...
heart_dat = readRDS(file = "{{path_heart_dat}}")
heart_dat$plt
...
sleep_ts = readRDS(file = "{{path_sleep_ts}}")
...
log_id = readRDS(file = "{{path_log_id}}")
...
if (!flag_no_gps_data) {
  ...
  res_tcx = readRDS(file = "{{path_res_tcx}}")
...
if (!flag_no_gps_data) {
  ...
  sf_rst_ext = readRDS(file = "{{path_sf_rst_ext}}")
}
...
if (!flag_no_gps_data) {
  ...
  raysh_rst = raster::raster("{{path_raysh_rst}}")
}
